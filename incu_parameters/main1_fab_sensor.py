    ...
    def gotosendmsg(self):
                ...
                if line not in ["DHTxx test!",None]:
                    ...
                    if len(y)>3 :
                        ...
                        if len(humidity)>0:
                            ...
                            x=humidity[["resistance"]].copy()
                            y=humidity["relativehumidity"].copy()
                            from sklearn.linear_model import LinearRegression
                            linear=LinearRegression()
                            linear.fit(humidity[["resistance"]].values,humidity[["relativehumidity"]].values)
                            #linear.feature_names_in_=["age","weight","heatout","roomhum","roomtemp"]
                            y=np.array([[float(resistance)]])
        
                            prediction=linear.predict(y)
                            list1.append(str(prediction[0][0]))
        ...
        def handle(msg):
            ...
            if(msg['text'] == '/dht'):
                ...
                bot.sendMessage(chat_id, "current humidity found:"+ str(r[2]))
